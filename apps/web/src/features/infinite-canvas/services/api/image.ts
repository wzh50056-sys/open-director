...
import { isMimoChannel, mimoModels } from "@/features/infinite-canvas/lib/mimo-tts";
import { imageToDataUrl, resolveImageUrl } from "@/features/infinite-canvas/services/image-storage";
import { buildApiUrl, channelIdForActiveModel, channelProtocolForConfig, directAIProviderForConfig, localChannelForActiveModel, type AiConfig } from "@/features/infinite-canvas/stores/use-config-store";
...
async function requestImageGenerationSingle(config: AiConfig & { seedIndex?: number; seedCount?: number }, prompt: string, params: ImageRequestParams): Promise<GeneratedImage[]> {
    ...
    if (directProvider) {
        const { requestDirectImages } = await import("@/features/infinite-canvas/services/api/direct-ai");
