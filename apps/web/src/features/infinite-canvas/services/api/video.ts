...
import { dataUrlToFile } from "@/features/infinite-canvas/lib/image-utils";
import { boolConfig, isSeedanceVideoConfig, normalizeSeedanceRatio } from "@/features/infinite-canvas/lib/seedance-video";
import { isKIEGrokVideoModel, isKIEKlingV3Config, kieKlingOmniVariant } from "@/features/infinite-canvas/components/video-settings-panel";
import { isCogVideoX3Model, modelKey, normalizeCogVideoX3Duration, supportsVideoAudioGeneration } from "@/features/infinite-canvas/lib/video-model-capabilities";
import { resolveMediaUrl, uploadMediaFile } from "@/features/infinite-canvas/services/file-storage";
import { imageToDataUrl, resolveImageUrl } from "@/features/infinite-canvas/services/image-storage";
import { buildApiUrl, channelIdForActiveModel, channelProtocolForConfig, directAIProviderForConfig, localChannelForActiveModel, type AiConfig, type VideoElementReference } from "@/features/infinite-canvas/stores/use-config-store";
...
import type { ReferenceAudio, ReferenceVideo } from "@/features/infinite-canvas/types/media";
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] | VideoReferenceInput = [], onProgress?: VideoProgressHandler, options?: string | VideoTaskCreateOptions): Promise<CreatedVideoGenerationTask> {
    ...
    try {
        ...
        const created = directProvider
            ? await (await import("@/features/infinite-canvas/services/api/direct-ai")).createDirectVideoTask(config, directProvider, body)
...
export async function pollCreatedVideoGenerationTask(config: AiConfig, task: VideoResponse, { startedAt = Date.now(), requestBody, initialDelayMs = 0, onProgress, onPoll }: { startedAt?: number; requestBody?: unknown; initialDelayMs?: number; onProgress?: VideoProgressHandler; onPoll?: (task: VideoResponse) => void } = {}) {
    ...
    const directPoll = directProvider ? (await import("@/features/infinite-canvas/services/api/direct-ai")).pollDirectVideoTask : null;
...
export async function pollVideoGenerationTaskStatus(config: AiConfig, task: VideoResponse) {
    ...
    const result = directProvider
        ? await (await import("@/features/infinite-canvas/services/api/direct-ai")).pollDirectVideoTask(config, directProvider, pollId)
