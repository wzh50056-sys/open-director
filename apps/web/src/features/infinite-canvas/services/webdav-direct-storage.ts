...
import type { UserWebDAVStorageProvider } from "@/features/infinite-canvas/services/image-storage";
...
export async function persistDirectWebDAV(provider: UserWebDAVStorageProvider, blob: Blob, filename: string) {
    ...
    try {
        ...
        try {
            const { registerDirectStorageObject } = await import("@/features/infinite-canvas/services/api/storage");
...
export async function deletePersistedDirectWebDAV(provider: UserWebDAVStorageProvider, storageKey: string) {
    ...
    const { deleteDirectStorageObjectRecord, getStorageObjectInfo } = await import("@/features/infinite-canvas/services/api/storage");
