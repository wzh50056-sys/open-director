...
import { deleteAnonymousStorageFile, uploadAnonymousStorageFile } from "@/features/infinite-canvas/services/anonymous-storage";
import { apiGet } from "@/features/infinite-canvas/services/api/request";
import { canUseGlobalStorage, getProxyUrl, loadUserStorageProvider, toProviderPayload, type StorageConfig, type UserWebDAVStorageProvider } from "@/features/infinite-canvas/services/image-storage";
...
async function uploadWebDAVMediaDirect(blob: Blob, filename: string, provider: UserWebDAVStorageProvider): Promise<UploadedFile | null> {
    const direct = await import("@/features/infinite-canvas/services/webdav-direct-storage");
...
export async function resolveMediaUrl(storageKey?: string, fallback = "") {
    ...
    if (storageKey.startsWith("server:")) {
        ...
        const { getStorageObjectInfo } = await import("@/features/infinite-canvas/services/api/storage");
...
export async function deleteStoredMedia(keys: Iterable<string>) {
    const { useAssetStore } = await import("@/features/infinite-canvas/stores/use-asset-store");
