...
import { localForageStorage } from "@/features/infinite-canvas/lib/localforage-storage";
import { cleanupUnusedImages, resolveImageUrl, uploadImage } from "@/features/infinite-canvas/services/image-storage";
import { cleanupUnusedMedia, resolveMediaUrl } from "@/features/infinite-canvas/services/file-storage";
import { fetchUserAssetData, syncUserAssetData } from "@/features/infinite-canvas/services/api/user-config";
                ...
                set((state) => {
                    ...
                    if (deletedAsset && deletedAsset.kind !== "text" && deletedAsset.data.storageKey) {
                        ...
                        window.setTimeout(async () => {
                            const { useCanvasStore } = await import("@/features/infinite-canvas/app/canvas/stores/use-canvas-store");
                            ...
                            const { collectImageStorageKeys } = await import("@/features/infinite-canvas/services/image-storage");
                            const { collectMediaStorageKeys } = await import("@/features/infinite-canvas/services/file-storage");
                            ...
                            if (!usedKeys.has(key)) {
                                if (key.startsWith("image:") || key.startsWith("server:")) {
                                    const { deleteStoredImages } = await import("@/features/infinite-canvas/services/image-storage");
                                    await deleteStoredImages([key]);
                                }
                                if (key.startsWith("file:") || key.startsWith("video:") || key.startsWith("server:")) {
                                    const { deleteStoredMedia } = await import("@/features/infinite-canvas/services/file-storage");
