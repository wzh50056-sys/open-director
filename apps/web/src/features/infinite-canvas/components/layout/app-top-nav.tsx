...
import { AppConfigModal } from "@/features/infinite-canvas/components/layout/app-config-modal";
import { MobileNavDrawer } from "@/features/infinite-canvas/components/layout/mobile-nav-drawer";
...
export function AppTopNav() {
    ...
    const isImageWorkbench = slug === "image";
    const isCanvasLibrary = pathname === "/canvas" || pathname === "/canvas/";
    const visibleNavigationTools = isCanvasLibrary
        ? []
        : isImageWorkbench
    ...
    return (
        <>
                ...
                <header className="sticky top-0 z-20 h-16 shrink-0 border-b border-stone-200 bg-background/90 backdrop-blur-xl dark:border-stone-800">
                    <div className="mx-auto flex h-full max-w-7xl items-stretch justify-between gap-5 px-6">
                        <div className="flex min-w-0 items-center">
                            ...
                            <nav className={cn("hide-scrollbar hidden h-16 min-w-0 items-center gap-7 overflow-x-auto md:flex", !isImageWorkbench && "ml-8")}>
            ...
            {visibleNavigationTools.length ? (
                <MobileNavDrawer open={mobileNavOpen} activeToolSlug={activeToolSlug} imageWorkbenchOnly={isImageWorkbench} onClose={() => setMobileNavOpen(false)} />
            ) : null}
            <AppConfigModal />
        </>
    );
